...
export default function BeyondPresenceAvatar({
  ...
  agentId: propAgentId = process.env.NEXT_PUBLIC_BEY_AGENT_ID || "",
  isSpeaking = false,
  isListening = false,
  className = "",
  onReady,
  ...
  // Use a ref to track if we've already processed the agentId
  ...
  useEffect(() => {
    ...
    console.log("BeyondPresenceAvatar check:", { 
      propAgentId, 
      hasValue: !!propAgentId,
      length: propAgentId?.length
    });

    // If we have a valid agentId, use it
    if (propAgentId && propAgentId.trim() !== '') {
      console.log(`✅ Loading agent with ID: ${propAgentId}`);
      setIsLoading(true);
  ...
  }, [propAgentId, avatarId, onReady, onError, useFallback]);
  ...
  // If agent ID is provided, use iframe embed
  if (propAgentId && propAgentId.trim() !== '') {
    return (
      <div className={`relative ${className}`}>
        {isLoading && (
        ...
        >
          <iframe
            ref={iframeRef}
            src={`https://bey.chat/${propAgentId}`}
